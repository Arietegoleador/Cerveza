...
function sumRange(start,end){
  return daily.filter(r=>r.date>=iso(start)&&r.date<=iso(end)).reduce((a,r)=>a+r.cl,0)
}
...
function liters(v){return `${(v/100).toFixed(2).replace(".",",")} L`}

function renderHome(){
  const now=new Date();
  const today=iso(now), ws=startOfWeek(now), we=addDays(ws,6);
...
function drawChart(canvas, values, labels, opts={}){
  ...
  pts.forEach((q,i)=>{ctx.beginPath();ctx.arc(q.x,q.y,5,0,Math.PI*2);ctx.fillStyle="#fffdf9";ctx.fill();ctx.strokeStyle="#6f1830";ctx.lineWidth=3;ctx.stroke()});
}
...
function renderStats(){
  ...
  if(statsTab==="monthly"){renderMonthly(box,base);return}
  renderWeekly(box);
}
function renderWeekly(box){
  const idx=WEEKLY.length-1+statsOffset;
  const currentStart=idx>=0&&idx<WEEKLY.length?dateObj(WEEKLY[idx].start):null;
  const currentEnd=currentStart?dateObj(WEEKLY[idx].end):null;
  let title=currentStart?`${fmtDate(iso(currentStart))} – ${fmtDate(iso(currentEnd))}`:"Semana sin datos históricos";
  const total=currentStart?WEEKLY[idx].cl:0;
  const weeks=[...WEEKLY];
  const ws=startOfWeek(new Date());
  const currentTotal=sumRange(ws,addDays(ws,6));
  weeks.push({start:iso(ws),end:iso(addDays(ws,6)),cl:currentTotal});
  const recent=weeks.slice(Math.max(0,weeks.length-10+statsOffset),weeks.length+statsOffset).slice(-10);
  box.innerHTML=`<div class="stat-card"><div class="period-nav"><button class="arrow" data-shift="-1">‹</button><div class="period">${title}</div><button class="arrow" data-shift="1">›</button></div>
    <div class="stat-grid">
      <div class="stat"><div class="l">CONSUMO</div><div class="v">${cl(total)}</div></div>
      <div class="stat"><div class="l">MÁXIMO SEMANAL</div><div class="v">${cl(Math.max(...WEEKLY.map(w=>w.cl)))}</div></div>
      <div class="stat"><div class="l">DÍAS CON CONSUMO</div><div class="v">${currentStart?countDays(currentStart,currentEnd):"—"}</div></div>
      <div class="stat"><div class="l">DÍAS SIN CONSUMO</div><div class="v">${currentStart?7-countDays(currentStart,currentEnd):"—"}</div></div>
    </div>
    <div class="chart-large"><canvas id="weeklyStatsChart"></canvas></div></div>`;
  const vals=recent.map(w=>w.cl), labs=recent.map(w=>fmtDate(w.start));
  drawChart($("#weeklyStatsChart"),vals,labs,{fill:true,limit:histAvg()});
  $$(".arrow").forEach(b=>b.onclick=()=>{statsOffset+=Number(b.dataset.shift);renderStats()});
}
function countDays(s,e){let n=0;for(let i=0;i<7;i++)if(getDaily(iso(addDays(s,i)))>0)n++;return n}
function renderMonthly(box,base){
  const months=[];
  for(let i=-7;i<=0;i++){const d=new Date(base);d.setMonth(base.getMonth()+i);const start=new Date(d.getFullYear(),d.getMonth(),1,12);const end=new Date(d.getFullYear(),d.getMonth()+1,0,12);months.push({start,end,total:monthlyTotal(start,end)})}
  const target=new Date(base); target.setMonth(base.getMonth()+statsOffset);
  const ms=new Date(target.getFullYear(),target.getMonth(),1,12), me=new Date(target.getFullYear(),target.getMonth()+1,0,12);
  const total=monthlyTotal(ms,me);
  const all=months.map(m=>m.total);
  box.innerHTML=`<div class="stat-card"><div class="period-nav"><button class="arrow" data-shift="-1">‹</button><div class="period">${monthName(ms)}</div><button class="arrow" data-shift="1">›</button></div>
    <div class="stat-grid">
      <div class="stat"><div class="l">CONSUMO</div><div class="v">${cl(total)}</div></div>
      <div class="stat"><div class="l">MÁXIMO MENSUAL</div><div class="v">${cl(Math.max(...all))}</div></div>
      <div class="stat"><div class="l">DÍAS CON CONSUMO</div><div class="v">${countMonthDays(ms,me)}</div></div>
      <div class="stat"><div class="l">DÍAS SIN CONSUMO</div><div class="v">${me.getDate()-countMonthDays(ms,me)}</div></div>
    </div><div class="chart-large"><canvas id="monthlyChart"></canvas></div></div>`;
  drawChart($("#monthlyChart"),all,months.map(m=>m.start.toLocaleDateString("es-ES",{month:"short"})),{fill:true});
  $$(".arrow").forEach(b=>b.onclick=()=>{statsOffset+=Number(b.dataset.shift);renderStats()});
}
function monthlyTotal(start,end){
  const direct=daily.filter(r=>r.date>=iso(start)&&r.date<=iso(end)).reduce((a,r)=>a+r.cl,0);
  // For July/August, split the weekly historical aggregate by days across month boundaries.
  const weeklyPart=WEEKLY.reduce((a,w)=>{
    const s=dateObj(w.start),e=dateObj(w.end);
    const overlapStart=new Date(Math.max(s, start)), overlapEnd=new Date(Math.min(e,end));
    if(overlapStart<=overlapEnd){const days=Math.round((overlapEnd-overlapStart)/86400000)+1; a+=w.cl*(days/7)}
    return a;
  },0);
...
function renderRecords(box){
  ...
  const monthly=monthsTotals();
  const maxMonth=Math.max(...monthly.map(x=>x.total),0);
  const streakDrink=maxStreak(true), streakDry=maxStreak(false);
  const minWeek=Math.min(...WEEKLY.map(w=>w.cl),sumRange(startOfWeek(new Date()),addDays(startOfWeek(new Date()),6)));
...
function monthsTotals(){return [6,7].map(m=>{const s=new Date(2026,m,1,12),e=new Date(2026,m+1,0,12);return {total:monthlyTotal(s,e)}})}
function maxStreak(drinking){
  if(!daily.length)return 0;
  const ds=[...new Set(daily.map(r=>r.date))].sort();
  let max=0,cur=0,last=null;
  for(const s of ds){
    const d=dateObj(s); const contiguous=last&&Math.round((d-last)/86400000)===1;
    const yes=getDaily(s)>0;
...
function renderHistory(){
  ...
  const rows=daily.filter(r=>r.cl>0).sort((a,b)=>b.date.localeCompare(a.date));
  if(!rows.length){list.innerHTML='<div class="empty">Todavía no hay registros.</div>';return}
  const groups={};
  rows.forEach(r=>{const k=dateObj(r.date).toLocaleDateString("es-ES",{month:"long",year:"numeric"});(groups[k]??=[]).push(r)});
...
function openModal(id=null){
  editingId=id;
  const r=id?daily.find(x=>x.id===id):null;
...
$("#historyAddBtn").onclick=()=>openModal();
$("#closeModal").onclick=closeModal;$("#cancelBtn").onclick=closeModal;$("#saveBtn").onclick=saveModal;
